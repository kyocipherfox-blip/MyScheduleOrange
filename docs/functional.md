...
- トグルボタン（`#controlToggleBtn`）でコントロールバー（`.interval-control`）を折りたたみ/展開できる
- 折りたたみ状態は `localStorage`（`schedule_v5`）の `controlsCollapsed` フィールドに保存・復元される
- 折りたたみ時はボタンラベルが「▼ コントロール」、展開時は「▲ コントロール」と表示される
